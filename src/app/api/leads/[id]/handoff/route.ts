...
import { answerFor, parseMoneyString } from "@/lib/leadgen/qualification";
...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  const currentRent = parseMoneyString(answerFor(qualification, "currentRent"));
  const taxes = parseMoneyString(answerFor(qualification, "taxes"));
  ...
  const dealData = serializeJsonFields(
    {
      ...
      seller: {
        ...
        termsResponse: answerFor(qualification, "termsResponse"),
        leadId: lead.id,
      },
